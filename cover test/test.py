...
# 导入os库
import os
folder_path = "sd render 20" # 这是你的文件夹路径
files = os.listdir(folder_path) # 这是一个包含所有文件名的列表
files_sizes = [(f, os.path.getsize(os.path.join(folder_path, f))) for f in files] # 这是一个包含所有文件名和大小的元组列表
files_sizes.sort(key=lambda x: x[1]) # 这会按照第二个元素（即文件大小）对列表进行升序排序
# for i, (f, s) in enumerate(files_sizes): # 这会遍历列表中的每个元组，并且用i来记录索引
#   old_name = os.path.join(folder_path, f) # 这是原来的文件路径和名字
#   ext = os.path.splitext(f)[1] # 这是文件的扩展名，例如.jpg
#   new_name = os.path.join(folder_path, f"{i+1:03d}{ext}") # 这是新的文件路径和名字，用三位数字和原来的扩展名
#   os.rename(old_name, new_name) # 这会重命名文件
# 导入random库
import random
# 定义间隔大小（单位为像素）
gap = 10
